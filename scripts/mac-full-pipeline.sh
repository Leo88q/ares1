#!/usr/bin/env bash
# ARES-1: обновление, полная проверка, сборка и deploy в Solana Devnet.
# Запуск из папки проекта: ./scripts/mac-full-pipeline.sh
set -Eeuo pipefail
...
[[ "$CURRENT_BRANCH" == "$BRANCH" ]] || fail "Ожидалась ветка $BRANCH, сейчас: $CURRENT_BRANCH"
git diff --quiet || fail "Есть незакоммиченные изменения. Сначала сохрани или убери их."
...
[[ -f "$HOME/.config/solana/id.json" ]] || fail "Нет кошелька ~/.config/solana/id.json"
solana address
solana balance

step "Полный локальный CI: тесты, сборка, validator, IDL"
cd "$ROOT_DIR/game"
./scripts/ci-local.sh
...
echo ""
echo "✅ Готово: проверки пройдены, контракт задеплоен в Devnet и инициализирован."
