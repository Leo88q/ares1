...
function Fastener({
  ...
  return (
    <g transform={`translate(${point.x} ${point.y})`}>
      <path
        d="M-4 -2.3 0 -4.6 4 -2.3V2.3L0 4.6 -4 2.3Z"
        fill="#090A0E"
        stroke="#756F78"
        strokeWidth="0.8"
      />
      <path
        d="M-2 -1.2 0 -2.3 2 -1.2V1.2L0 2.3 -2 1.2Z"
        fill="#35343D"
        stroke="#A39BA4"
        strokeOpacity="0.45"
        strokeWidth="0.65"
...
export function AresHullFrame({
  ...
  useEffect(() => {
    ...
    return subscribeFrames((deltaSeconds) => {
      ...
      const opacity = moving
        ? Math.sin(progress * Math.PI) * 0.9
        : 0;

      traceOpacity.set(opacity);
    });
  }, [
    engaged,
    visible,
      ...
      >
        ...
        {!compact &&
          geometry.fasteners.map((point, index) => (
            <Fastener key={index} point={point} />
          ))}
        ...
        {!reducedMotion && runningLight && (
          <motion.path
            ref={(element) => {
              (traceRef as React.MutableRefObject<SVGPathElement | null>).current = element;
