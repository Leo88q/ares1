  ...
  <div role="status" style={{ gridColumn: '1 / -1', padding: '60px 20px', textAlign: 'center' }}>
   {error ? (
    <>
     ...
     <h2 style={{ fontSize: 20, marginBottom: 12 }}>Нет связи с блокчейном</h2>
     <p style={{ color: 'var(--pf-text-secondary)', fontSize: 14 }}>{error}</p>
     <p style={{ color: 'var(--pf-text-muted)', fontSize: 12, marginTop: 8 }}>Повторяем автоматически…</p>
    </>
   ) : (
    <>
     <Loader2 size={48} color="var(--pf-teal)" style={{ marginBottom: 20, animation: 'spin 1s linear infinite' }} />
     <h2 style={{ fontSize: 20, marginBottom: 12 }}>Инициализация игры…</h2>
     <p style={{ color: 'var(--pf-text-secondary)', fontSize: 14 }}>Подключаемся к блокчейну</p>
    </>
   )}
  </div>
 )
...
function ConnectHint() {
 return (
  <div style={{ gridColumn: '1 / -1', padding: '40px 20px', textAlign: 'center' }}>
   ...
   <h2 style={{ fontSize: 20, marginBottom: 8 }}>Подключи кошелёк</h2>
   <p style={{ color: 'var(--pf-text-secondary)', fontSize: 14 }}>Поля хранятся on-chain — чтобы увидеть ферму, нажми «Подключить кошелёк» сверху.</p>
  </div>
 )
}
...
function BuyFieldCard({ onPurchase, purchasing, balanceMicro, firstField }: BuyProps) {
 ...
 return (
  ...
  >
   <div className="ares-stencil" style={{ fontSize: 13, color: 'var(--ares-hud-amber, #FFB347)', marginBottom: 10, textShadow: '0 0 10px rgba(255,179,71,0.4)' }}>КУПИ РАСТЕНИЕ</div>
   <MiniHydroModules />
   <p style={{ fontSize: 14, color: 'var(--pf-text-secondary)', marginBottom: 4, textAlign: 'center' }}>
    {firstField ? 'Заложи первую делянку' : 'Заложи новую жизнь'}
   </p>
   {firstField && (
    <p style={{ fontSize: 11, color: 'var(--pf-text-muted)', marginBottom: 12, textAlign: 'center' }}>
     Стартовый паёк — в «Каюте» (задачи смены) или на бирже за SKR.
    </p>
   )}
   <div style={{ display: 'flex', flexDirection: 'column', gap: 8, width: '100%' }}>
    {FIELD_TYPES.map((t) => {
     const price = fieldPriceMicro(t.id)
     ...
     return (
      <motion.button
       ...
       aria-label={`Купить ${t.name} за ${fmtPotato(price, 0)} POTATO`}
       onClick={() => {
        sounds.buy()
        haptics.purchaseField()
        void onPurchase(t.id)
       }}
       style={{ padding: 10, borderRadius: 10, fontSize: 12, fontWeight: 600, display: 'flex', justifyContent: 'space-between', ...styles[t.id] }}
      >
       <span><span className="ares-mono" style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.14em' }}>{RARE_LABELS[t.id]}</span> <span className="ares-mono" style={{ fontSize: 10, opacity: 0.75 }}>(×{(t.yieldBps / 10_000).toFixed(2)})</span></span>
