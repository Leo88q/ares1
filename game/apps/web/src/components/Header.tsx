 ...
 return (
  <header>
   <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, gap: 12 }}>
    <div>
     <h1 className="pf-h1" style={{ fontSize: 26 }}>АГРО-ОТСЕК</h1>
     <p className="pf-subtitle" style={{ marginTop: 4 }}>Теплица на Марсе под фитолампами</p>
    </div>
    <div data-tutorial="balance" style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 14 }}>
      <div>
        ...
        <motion.div whileHover={{ scale: 1.03 }} className="pf-card hull-skin" style={{ padding: '10px 22px', borderRadius: 12, minWidth: 176, display: 'flex', justifyContent: 'center', border: '1px solid rgba(160, 82, 40, 0.65)' }} aria-label="Баланс POTATO">
   ...
   <div style={{ marginBottom: 16 }}>
    {connected && publicKey ? (
     <motion.button
      ...
      aria-label="Смена кошелька кошелёк"
     ...
     >
      ...
      <span style={{ fontSize: 11, color: 'var(--pf-text-muted)' }}>· Смена кошелька</span>
     </motion.button>
    ) : (
     <motion.button
      whileTap={{ scale: 0.98 }}
   ...
   <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
    <StatCard label="РАНГ" value={stats.playerLevel} />
    <StatCard label="РАСТЕНИЙ" value={stats.totalFields} />
    <StatCard label="СТАЖ" value={Math.floor(stats.experience)} />
   </div>
  </header>
 )
}
